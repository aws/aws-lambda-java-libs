...
public class GraalVMSupportTest {
    ...
    private static final File CONFIG_LOCATION = FileUtils.getFile("src", "main", "resources", "META-INF", "native-image", "com.amazonaws", "aws-lambda-java-events", "reflect-config.json");
    ...
    public void testThatAllEventClassesExistWithinGraalVMReflectConfig() throws IOException, ClassNotFoundException {
        ...
        Assertions.assertEquals(nonTestClassesInPackage.size(), actualReflectConfigEntries.size(), "Please add the new event / response to the reflect-config.json");
    }
}
